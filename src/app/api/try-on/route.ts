...
function buildVtonPrompt(garment: {
  ...
  if (garment.description || quickTags.length > 0) {
    catalogBlock =
      "AUTHORITATIVE CATALOG INFORMATION ABOUT IMAGE 2 — a human reviewer has already examined this garment photo and recorded what it actually shows. Trust this over your own visual read of Image 2 wherever the two would differ, since it may name details (exact brand text, precise colors, which individual pieces are present) that are hard to read directly from the photo. Image 2 may show a single standalone item or a multi-piece look - treat the catalog information as the definitive list of pieces and styling. ";
    if (quickTags.length > 0)
      catalogBlock += `Category/fit tags: [${quickTags.join("; ")}]. `;
    if (garment.description)
      catalogBlock += `Full description and any styling instructions: ${garment.description} `;
  }
  ...
  return (
    ...
    "GARMENT SCOPE & CROP PRIORITY — apply only the catalog items that naturally fall WITHIN the visible camera framing of Image 1. If Image 1 cuts off at the chest, waist, or knees, render only the parts of the clothing visible within that exact crop. DO NOT force off-frame items (such as shoes, boots, or lower pant hems) into the image, and NEVER shrink, squish, or compress the person's body, neck, or limbs to make them fit. Let items cut off at the edge of the frame exactly as the person does in Image 1. Outside the replaced region, everything must remain exactly as shown in Image 1. " +
    "LAYERING AND POSITIONING — when multiple pieces are involved, follow any explicit layering/tucking/positioning instruction from the catalog information above first. Only when no such instruction is given, use the most natural, conventional way those pieces would actually be worn together. " +
    "FIT AND SILHOUETTE — follow any fit/styling directive from the catalog information above first. Reproduce that exact fit character on the person's actual body — do not normalize a loose garment into a tight one or vice versa, and do not default to a generic 'flattering' fit. The garment should hang, cling, or structure itself on this specific body exactly as dictated, adjusted only for this person's proportions and pose. " +
    "FABRIC PHYSICS — render drape, tension, and folds consistent with the fit above and the fabric type visible in Image 2. Loose fabric should show soft gathering; tight fabric should show tension lines; structured fabric should hold its own shape. Shadows and highlights on the garment must match Image 1's existing light source and direction exactly. " +
    ...
    "SKIN VISIBILITY CHANGES — if the new garment's silhouette exposes skin that was covered in Image 1, render that skin plainly matching tone and texture from Image 1. If it covers skin, let it cover naturally. " +
    "ACCESSORY REPLACEMENT — only if Image 2 explicitly includes an accessory, replace the person's existing item in that same category. If none is specified, change none. " +
    "SCENE & CAMERA INTEGRITY (STRICT 1:1 FRAMING LOCK) — keep the camera angle, focal length, camera distance, framing edges, subject scale, background, and lighting 100% IDENTICAL to Image 1. Do NOT zoom in (do not crop closer into the subject) and do NOT zoom out (do not pull back or widen the view). The person must occupy the exact same pixel dimensions, position, and scale on the canvas as in Image 1. If the person's feet or lower legs are not in Image 1, they must NOT appear in the output. " +
    ...
    "DO NOT: zoom in or crop closer to the subject; zoom out or pull back the camera; change subject scale or camera distance; compress or shrink the body/height to fit footwear; render feet/shoes if not visible in Image 1; alter identity, body shape, proportions, or pose; add or remove any physical detail not visible in Image 1; apply a fit, layering, or drape that contradicts the catalog information; touch any body region or clothing item outside the garment's actual scope; change background or lighting; leave tags, hangers, or extraneous hands in frame; output more than one image or any text."
  );
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const [idx, model] of MODELS_PRIORITY.entries()) {
      ...
      try {
        console.log(
          "jusssssssssssssssssssssssssssssssssssssssssssssssssssssssss",
          promptText,
        );
