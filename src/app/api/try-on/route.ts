...
function buildVtonPrompt(garment: {
  ...
  return (
    ...
    "GARMENT SCOPE — if the catalog information above lists specific pieces, use exactly that list - apply every piece it names (whether that's one item or several) and nothing else, regardless of what else might be faintly visible in Image 2. except where the FRAME BOUNDARY RULE below excludes a piece because its body region isn't visible in Image 1. If no catalog information was given, infer the coverage directly from Image 2: whether it is a top only, bottom only, full outfit/suit/set, outer layer worn over other clothing, footwear, or an accessory. Either way, replace ONLY the body region(s) those piece(s) actually occupy. Every part of the person's body, clothing, and accessories outside that region must remain exactly as shown in Image 1 — do not restyle, recolor, or regenerate anything you weren't asked to change. " +
    "FRAME BOUNDARY RULE — Image 1's existing crop defines the absolute boundary of the output. Never extend, zoom out, or invent anatomy beyond what is already visible in Image 1, even if a catalog garment piece would normally require it. If a piece (e.g. shoes) corresponds to a body region not visible in Image 1's frame, skip that piece entirely and proceed only with pieces whose body regions are visible. Never resize, shrink, or rescale the visible person to make room for anatomy or garment pieces that would otherwise fall outside the original frame." +
