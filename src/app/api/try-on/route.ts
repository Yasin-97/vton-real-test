...
function buildVtonPrompt(garment: {
  ...
  return (
    ...
    "GARMENT SCOPE — if the catalog information above lists specific pieces, use exactly that list - apply every piece it names that falls WITHIN the camera frame of Image 1. If Image 1 cuts off at the chest, waist, or knees, render only the parts of the clothing visible within that exact framing. Never compress, shrink, or zoom out the person to show footwear or pants hems that were not visible in the original photo., regardless of what else might be faintly visible in Image 2. If no catalog information was given, infer the coverage directly from Image 2: whether it is a top only, bottom only, full outfit/suit/set, outer layer worn over other clothing, footwear, or an accessory. Either way, replace ONLY the body region(s) those piece(s) actually occupy. Every part of the person's body, clothing, and accessories outside that region must remain exactly as shown in Image 1 — do not restyle, recolor, or regenerate anything you weren't asked to change. " +
    ...
    "CAMERA FRAMING & CROP PRIORITY (CRITICAL):" +
    "- Image 1's exact camera shot, crop, and framing are SACRED. Do NOT zoom out, do NOT change the camera distance, and do NOT squeeze the body to fit items into the frame." +
    "- CROP OVERRIDE: If the catalog describes an item (e.g., shoes, socks, pants) that falls OUTSIDE the visible framing of Image 1 (e.g., if Image 1 cuts off at the waist or knees), DO NOT force that item into the picture. Simply let the frame cut off naturally at the exact same point as Image 1." +
    "- Never outpaint or compress the person's anatomy, neck, or limbs to reveal items that are below or outside the frame edge.'SOURCE CLEANUP — exclude anything from Image 2 that isn't the garment itself: hangers, mannequin parts, model's hands, price tags, brand stickers. " +
